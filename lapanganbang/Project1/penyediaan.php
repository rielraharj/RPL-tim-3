...
<table border="1" cellspacing="0" cellpadding="4">
 <tr style="text-align:center;background-color:#ffffff">
  ...
  <?php
  $no = 1;
  $penyedia = mysqli_query($conn, "SELECT id_penyedia FROM penyedia_lapangan WHERE id_user='$id_user']");
  
  $getpenyedia = mysqli_fetch_assoc($penyedia);
  $sedia = $getpenyedia['id_penyedia'];
  $data = mysqli_query($conn, "SELECT * FROM lapangan WHERE id_penyedia='$sedia'") or die (mysqli_error());
